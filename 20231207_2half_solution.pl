...
sub getSortKey4Hand($){
	...
	# because of the J == Joker upgrade the type
	if(exists($h{J}) and $h{J} < 5){ # I have some Jokers; 5 Jokers don't change anything, it's the highest type however
		my $nJ = delete($h{J});
		my $keyMaxValue;
		my $maxValue = 0;
		foreach(keys(%h)){
	...
	my $sortKey = $hand;
	$sortKey =~ tr/AKQJT/ZYX1V/; # J is smaller than 2
