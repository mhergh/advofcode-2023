...
def get_min_dist_between_galaxies(g1, g2):
    ...
    d_min += abs(dichotomic_search(idx_empty_row, g1[0])[0] - dichotomic_search(idx_empty_row, g2[0])[0])         # number of empty rows between g1 and g2; only the first returned value of the function is relevant
    d_min += abs(dichotomic_search(idx_empty_column, g1[1])[0] - dichotomic_search(idx_empty_column, g2[1])[0])   # number of empty cols between g1 and g2; only the first returned value of the function is relevant
    global dist_galaxy
    dist_galaxy[(g1, g2)] = d_min
    return d_min

def get_total_min_dist_comb2_galaxies(p_g):
    sum = 0
    for idx_g1 in range(0, len(p_g) - 1):
        for idx_g2 in range(idx_g1 + 1, len(position_galaxy)):
            sum += get_min_dist_between_galaxies(p_g[idx_g1], p_g[idx_g2])
    return sum
...
if __name__ == '__main__':
    ...
    sum_comb2_galaxies = get_total_min_dist_comb2_galaxies(position_galaxy)
